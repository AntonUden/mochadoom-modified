...
/**
 * Display, its configuration and resolution related stuff, DoomFrame creation,
 * full-screen related code. Window recreation control. That sort of things.
 */
public class DoomWindowController<E extends Component & DoomWindow<E>, H extends Enum<H> & EventBase<H>> implements FullscreenOptions {
	...
	private IDoomFrame doomFrame;

	/**
	 * Default window size. It might change upon entering full screen, so don't
	 * consider it absolute. Due to letter boxing and screen doubling, stretching
	 * etc. it might be different that the screen buffer (typically, larger).
	 */
	...
	DoomWindowController(
		...
		this(handlerClass, device, imageSource, doomEventConsumer, component, defaultWidth, defaultHeight, null);
	}
	...
	DoomWindowController(
		...
		if (jframe == null) {
			System.out.println("Init DoomFrame");
			this.doomFrame = new DoomFrame<>(dimension, component, imageSource);
		} else {
			System.out.println("Init ExistingJFrameDoomFrame");
			this.doomFrame = new ExistingJFrameDoomFrame<>(jframe, dimension, component, imageSource);
		}
	...
	public boolean switchFullscreen() {
		...
		doomFrame.dispose();
		doomFrame = new DoomFrame<>(dimension, component, doomFrame.getImageSupplier());
	...
	/**
	 * FULLSCREEN SWITCH CODE TODO: it's not enough to do this without also
	 * switching the screen's resolution. Unfortunately, Java only has a handful of
	 * options which depend on the OS, driver, display, JVM etc. and it's not
	...
	public final boolean switchToFullScreen() {
		...
		// Full-screen mode
		device.setFullScreenWindow(isFullScreen ? (Window) doomFrame : null);
	...
	private void updateSize() {
		doomFrame.getJFrame().setPreferredSize(isFullscreen() ? dimension : null);
